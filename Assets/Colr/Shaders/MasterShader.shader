	...
	SubShader {
		...
		Tags { "LIGHTMODE"="ForwardBase" "QUEUE"="Geometry" "RenderType"="Opaque" }
			...
			fixed4 frag(v2f v) : COLOR {
				float4 result = VEC4_ZERO;
				float4 lightColor = float4(_LightTint * v.lighting, 1);

                #ifdef LIGHTMAP_ON
                    #if LIGHTMAP_COLR_ON
                        float4 lmColor = UNITY_SAMPLE_TEX2D(unity_Lightmap, v.lightmap_uv) * SHADOW_ATTENUATION(v);
						unity_Lightmap_HDR = half4(4,0,0,0);
                        float4 dlm = float4(DecodeLightmap(lmColor), 1) * SHADOW_ATTENUATION(v);
                        float4 lmPower = lerp(VEC4_ONE, dlm, _LightmapPower);
                        result = lerp(half4(_LightmapColor, 1), VEC4_ONE, lmPower) * lightColor;
                    #else
